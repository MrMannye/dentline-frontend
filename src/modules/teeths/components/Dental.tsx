            ...
            <div className="relative p-4 w-full ml-2 mt-10 lg:mt-0 lg:ml-7">
                {/* <div className=''> */}
                    ...
                    {Teeth[0].teeth.map(tooth => {
                        return (
                            <div key={tooth.number} className={`${!tooth.visible && "hidden"}`}>
                                ...
                                <img onClick={(e) => handleSelectTooth(tooth.number.toString())} src={`/img/sheet/p${tooth.number}.svg`} className={`D-${tooth.number} filter saturate-100`} alt={`D-${tooth.number}`} />
                            </div>
                        )
                    })}
                    {/* DIENTES NIÑO */}
                    {Teeth[1].teeth.map(tooth => {
                        return (
                            <div key={tooth.number} className={`${!tooth.visible && "hidden"}`}>
                                <p className={`num-teeth-${tooth.number} num-teeth`}>{tooth.number}</p>
                                <img onClick={(e) => handleSelectTooth(tooth.number.toString())} src={`/img/theet/${tooth.number}.png`} className={`D-${tooth.number}`} alt="dent_18" />
                            </div>
                        )
                    })}
                {/* </div> */}
