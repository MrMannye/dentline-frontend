...
function CardPayment({ motivo, costo_total, fecha_cita, abono, observaciones }: DatePacientRecap) {
    ...
    const updatedProcedures = procedimientos.filter((proc: any) => proc.id !== id);
    ...
    <div className='rounded-xl flex flex-col m-4 mt-8 border'>
      ...
      <div className='text-xs mx-4 my-2 text-gray-400 flex flex-col'>
        ...
        {motivo?.split("_").map((procedimiento, index) => {
          const [id, name, cost] = procedimiento.split("-");
          return (
						<div key={index} className='flex flex-row items-center mb-1 space-x-2'>
							<RemoveCircleOutlineIcon onClick={() => handleRemoveProcedure(id)} className='text-xs text-red-500'>Eliminar</RemoveCircleOutlineIcon>
							<span key={id}>{id} - {name} - ${cost}</span>;
						</div>
        )})}
      </div>
      <hr />
