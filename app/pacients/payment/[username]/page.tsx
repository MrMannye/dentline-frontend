...
export default function Payment({ params }: { params: { username: string } }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      parseProcedures(data.data[0].motivo);
    };
    fetchData();
    reset({ abono: 0 });
  }, [id_cita, reset]);

  const parseProcedures = (motivo: string) => {
    const parsed = motivo.split("_").map((proc, index) => {
      const [id, name, cost] = proc.split("-");
      return { id: index, name, cost: parseFloat(cost) };
    });
    setProcedimientos(parsed);
  };

  const handleAddProcedure = () => {
    if (newProcedure && newCost) {
      const newProc: Procedure = {
        id: procedimientos.length,
        name: newProcedure,
        cost: Number(newCost),
      };
  ...
  const handleRemoveProcedure = (id: number) => {
    const updatedProcedures = procedimientos.filter((proc) => proc.id !== id);
    setProcedimientos(updatedProcedures);
  };

  const generateMotivoString = () => {
    return procedimientos.map((proc) => `${proc.id}-${proc.name}-${proc.cost}`).join("_");
  };
